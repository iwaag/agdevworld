...
- `/api/autolab/projects` — POST `{"project":"<lowercase-hyphen name>","concept":"…"}` starts a new autolab project: the `autodev/<name>` + `<name>-direction` Gitea repo pair (direction seeded with GUIDE/concept/.gitignore), a fresh Plane project, and the standing `#pj-<name>` Zulip channel with every agent and the Developer subscribed. The hyphenated name is the contract everywhere except inside Plane, which refuses a hyphen in a project name: there it becomes the same words with spaces, under an identifier derived from the name (`whack-a-mole-2` → `WAM2`, `quiz` → `QUIZ`, at most 12 characters). Two names that derive the same identifier collide; the failure names both fields it sent. The answer carries the Plane project UUID and its state ids — keep them: mission briefings must quote them, because nodes are not configured per project. This creates no issue and starts no mission; development starts separately, on purpose.
