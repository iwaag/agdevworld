...
//
// `POST /api/chat` is one complete agent run. Server-capable tools execute
// inside that run; the response carries only prose and UI actions for this
// browser to apply.
...
export function initChatPanel(
  getContext: () => string,
  // UI-only actions this panel does not own, principally switch_view.
  ...
  function runAction(call: ToolCall): string {
  ...
  async function send(question: string) {
    ...
    try {
      ...
      pending.textContent = data.reply || 'OK.'
      messagesEl.scrollTop = messagesEl.scrollHeight
      const actions: AssistantAction[] = Array.isArray(data.actions) ? data.actions : []
      actions.forEach((action, index) => {
        const { action: name, ...arguments_ } = action
        runAction({ id: `${data.run?.id ?? 'run'}-${index}`, name, arguments: arguments_ })
      })
    } catch {
      pending.remove()
