...
export async function initOperationParts(): Promise<void> {
  ...
  async function select() {
    ...
    if ('error' in detail) { graph.textContent = `Unknown — ${detail.error}`; return }
  ...
  async function refresh() {
    ...
    else { sessions.replaceChildren(); graph.textContent = board.health.state === 'live' ? 'No routines observed.' : `Unknown — ${board.health.reason}` }
  }
